...
export const Header = () => {
  ...
  const mobileNav = clsx("fixed inset-y-0 left-28 right-0 isolate z-20 flex size-full flex-col gap-4 bg-white p-6 pt-12 shadow", { "translate-x-0": showMobileNav }, { "translate-x-[100%]": !showMobileNav });
  ...
  return (
    <header className="mb-20 p-4 md:p-6">
      <div className="flex items-center justify-between rounded border p-4 pl-6 md:rounded-full md:pl-10 xl:px-5 xl:py-4 xl:pl-12">
        <div>
          <img src={"/company_icon.svg"} className="w-[80px] md:w-[96px] xl:w-[120px]" />
        </div>
        ...
        <button onClick={() => setshowMobileNav((prev) => !prev)} className="flex size-[32px] items-center justify-center rounded border border-[#CACACA] p-0 md:hidden">
          <img src={"/menu-icon.svg"} alt="close icon" className="w-[16px] lg:w-auto" />
        </button>
